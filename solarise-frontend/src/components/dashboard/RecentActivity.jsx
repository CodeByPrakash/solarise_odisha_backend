...
const RecentActivity = () => {
  ...
  const getIcon = (type) => {
    if (type === 'payment') {
      return (
        <div className="h-9 w-9 rounded-[14px] bg-emerald-50 text-emerald-600 border border-emerald-200/60 flex items-center justify-center shrink-0 text-sm shadow-2xs">
          💰
        </div>
      );
    }
    if (type === 'document') {
      return (
        <div className="h-9 w-9 rounded-[14px] bg-purple-50 text-purple-600 border border-purple-200/60 flex items-center justify-center shrink-0 text-sm shadow-2xs">
          📄
        </div>
      );
    }
    return (
      <div className="h-9 w-9 rounded-[14px] bg-blue-50 text-blue-600 border border-blue-200/60 flex items-center justify-center shrink-0 text-sm shadow-2xs">
        📱
      </div>
    );
  };
      ...
      <div>
        ...
        <div className="flex justify-between items-center border-b border-slate-100 pb-4 mb-4">
          <div className="flex items-center space-x-2.5">
            <div className="h-8 w-8 rounded-xl bg-indigo-50 text-indigo-600 border border-indigo-200/60 flex items-center justify-center font-bold text-sm shadow-2xs">
              ⚡
            </div>
